...
#pragma endregion
	...
	SCIPwriteOrigProblem(RMPscipModel, "D://RMP_init.lp", NULL, NULL);
	//SCIPsolve(scipModel);
...
int MasterProb::Solve_Dual(Solution& duals)
{
	...
	SCIPwriteOrigProblem(RMPscipModel, "D://testRMP.lp", NULL, NULL);
	...
	if (SCIPgetStatus(RMPscipModel) == SCIP_STATUS_OPTIMAL)
	{
		...
		for (int i = 0; i < RMPscipConss_num; i++)
		{
			...
			////0517wqy新增测试begin
			//cout << RMPscipConss[i]->name << ":" << var << endl;
			////std::cout << "lhs:" << SCIPgetLhsLinear(RMPscipModel, RMPscipConss[i]) << std::endl;
			////std::cout << "rhs:" << SCIPgetRhsLinear(RMPscipModel, RMPscipConss[i]) << std::endl;
			//if (var > 100000)
			//{
			//	int a = 1;
			//}
...
{
	SCIPwriteOrigProblem(RMPscipModel, "D://lastRMP.lp", NULL, NULL);
	SCIPsetIntParam(RMPscipModel, "display/verblevel", 0);
	SCIPsolve(RMPscipModel);
