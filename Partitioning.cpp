    ...
    -> std::vector<Partition>
{
    ...
    const auto max_edge_size =graph.getEdgesizeOfPercentBiggestEdge(ignore_biggest_edges_in_percent);
    ...
    for(std::size_t i = 0; i < number_of_partitions; ++i) 
    {
        ...
        //std::unordered_set<int64_t> onn_Node = {vertex_v.begin()->first};
        SSet s_set{graph,
                   s_set_size,
                   num_neigs_flag,
                   node_select_flag};
        while(!is_partition_full(i, part) && !graph.getVertices().empty()) 
        {
            
            auto next_node = s_set.getNextNode();

            // insert node to C
            part.addNode(next_node,graph.getEdgesOf(next_node));
  ...
  //      std::unordered_set<int64_t> part_vertices = part.getNodes();
  //      for (auto vertex_id : part_vertices)
  //      {
  //          vertex_v.erase(std::remove_if(vertex_v.begin(), vertex_v.end(),
  //              [&](const auto& pair) { return pair.first == vertex_id; }));
		//}
        part_vec.push_back(std::move(part));
        std::cout << "µÚ" << i << "ÂÖ½áÊø" << std::endl;
    }
...
auto part::getHyperedgeCut(const std::vector<Partition>& partitions)
	-> std::future<std::unordered_set<int64_t>>
{
    return std::async(std::launch::async,
                      [&partitions]() -> std::unordered_set<int64_t> {
